 ...
 * 3. 每个 IDE 槽位拥有 endpoint_revision，自增于端点重绑，实现同级隔离；
 * 4. 纯数据模型与校验（纯函数/变换），不涉及状态机与生命周期守卫。
 */
...
export function createBinding({
  ...
  // 为兼容旧单端读取表面，在恰好 1 个 IDE 端点时定义 getter
  Object.defineProperty(binding, 'ide', {
    get() {
    ...
    set(val) {
      if (val && typeof val === 'object') {
        this.ide_endpoints = [{
          endpoint_id: val.endpoint_id || 'ide',
          endpoint_revision: val.endpoint_revision || 1,
          conversation_id: val.conversation_id || '',
          workspace_identity: val.workspace_identity || '',
          repository_identity: val.repository_identity || ''
        }];
      }
    },
    enumerable: true,
...
export function bumpRevision(binding) {
  ...
  Object.defineProperty(next, 'ide', {
    get() {
      if (this.ide_endpoints && this.ide_endpoints.length === 1) {
        return this.ide_endpoints[0];
      }
