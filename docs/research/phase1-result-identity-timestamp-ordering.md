...
1. **ChatGPT 真实会话、Reload 稳定性与分支导航深度探针**：
   ...
   - **[Verified + Inferred] 分支导航 vs 新完成歧义分析**：
     ...
     - [Verified] 新生成生命周期伴随 `button[data-testid="stop-button"]` 出现至消失，必须由持续在线监视（Live Witnessed）捕获；[Inferred / not directly exercised in this Rev 3 probe] 历史 branch navigation 是一个可能改变当前可见 Assistant result 而不提供该 live generation lifecycle 的来源。由于静态 DOM 不提供 provider-native completion time 或历史-event provenance，本研究没有证据足以把 observation gap 后的未知 Browser cursor 变化证明为“新 completion”。
...
| 分支导航 / 历史分支状态 | **当前 bounded probe 未建立可依赖 guard** [Verified] | 否 | UI/history navigation risk | 未形成稳定契约 | 否 | **UNSAFE 作为完成证明**：本轮没有证明存在一个可在 observation gap 后区分“新 completion”与“历史 branch/navigation 导致当前可见 result 改变”的稳定信号。 |
...
- **风险场景 [Inferred / product-behavior risk, not directly exercised in Rev 3]**：Rally 离线期间，当前可见 Browser result 可能因历史 branch/navigation 发生变化，而 Rally 没有见证新的 generation lifecycle；
- **已验证的歧义边界**：Rally 重新连接后，如果只看到 Browser 当前游标与持久化游标不同，静态 DOM 没有 provider-native completion time、event provenance 或本轮已验证的 branch-history guard，因此不能把该变化证明成“离线期间发生的新 completion”；
